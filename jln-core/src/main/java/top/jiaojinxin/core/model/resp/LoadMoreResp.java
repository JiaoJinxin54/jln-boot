...
public class LoadMoreResp<ID, T> extends SingletonResp<LoadMoreResp.LoadMoreResult<ID, T>> {
    ...
    public static <ID, T> LoadMoreResp<ID, T> ok(ID last, @NonNull T[] items) {
        return ok(new DefaultLoadMoreResult<>(last, Arrays.stream(items).collect(Collectors.toList())));
    }
    ...
    public static class DefaultLoadMoreResult<ID, T> implements LoadMoreResult<ID, T> {
        private static final long serialVersionUID = 7736833178584349662L;

        private final ID last;
